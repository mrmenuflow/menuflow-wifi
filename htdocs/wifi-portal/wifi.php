...
<!-- landscape content -->
<style>#mf-menu {display: block;}#rotate {display: none;height: 100%;overflow: hidden;touch-action: none;background: <?php echo $_SESSION['config']['navbar_bg_hex'];?>;}@media screen and (min-aspect-ratio:5/3){body {position: fixed;overflow: hidden;touch-action: none;background:<?php echo $_SESSION['config']['navbar_bg_hex'];?>;}#rotate {display: block;height: 100%;overflow: hidden;}#mf-menu {display: none;} }#rotate .splash {position: fixed;background: <?php echo $_SESSION['config']['navbar_bg_hex'];?>;width: 100%;height: 100%;z-index: 1060!important;-webkit-transform: translateZ(2000px);transform: translateZ(2000px);display: flex;justify-content: center;align-items: center;}#rotate .splash .content {display: flex;flex-direction: column;margin-left: 25px;color: #fff;font-family: "Manrope",sans-serif;}#rotate .splash .ls_title {display: inline-flex;font-size: 2.2rem;font-weight: 800;line-height: 38px;}#rotate .splash .ls_text {display: inline-flex;font-size: 1rem;font-weight: 500;padding-top: 20px;}</style>
